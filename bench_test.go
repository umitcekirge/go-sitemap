...
func benchProvider(name string, n int, decorate func(i int, e *Entry)) *FuncProvider {
	return &FuncProvider{
		...
		StreamFunc: func(ctx context.Context, yield func(Entry) error) error {
			for i := range n {
				e := Entry{Loc: "https://example.com/products/item-" + itoa(i)}
				if decorate != nil {
					decorate(i, &e)
				}
...
func BenchmarkExtensions50k(b *testing.B) {
	...
	benchRun(b, Options{}, 50_000, func(i int, e *Entry) {
		e.LastMod = &lm
		e.Images = []Image{{Loc: "https://cdn.example.com/img-" + itoa(i) + ".jpg", Title: "Item & Co."}}
		e.Alternates = []Alternate{
			{HrefLang: "en", Href: "https://example.com/en/item-" + itoa(i)},
			{HrefLang: "de", Href: "https://example.com/de/item-" + itoa(i)},
		}
	})
}
